"""
Transfer Learning with ResNet on SVHN (Street View House Number)
Fine-tuning pretrained ResNet for SVHN classification

This task is based off cnn_lvl3_resnet_transfer task with changes including a new dataset, the SVHN dataset.
It maintains PyTorch task structure, uses a pretrained ResNet18, training first the classifier head with a frozen backbone, then fine-tuning the model.
"""
...
# Output directory
OUTPUT_DIR = "/Developer/AIserver/output/tasks/cnn_resnet_transfer_svhn"
...
def get_task_metadata():
    ...
    return {
        "task_name": "cnn_resnet_transfer_svhn",
        ...
        "input_shape": [3, 32, 32],
        "description": "Fine-tuning pretrained ResNet18 on SVHN dataset",
    }
...
def make_dataloaders(batch_size=64, val_ratio=0.2, num_workers=2):
    """
    Create data loaders for SVHN dataset.
    ...
    # ImageNet normalization for transfer learning with pretrained ResNet
    ...
    # Load full SVHN training dataset
    full_train_dataset = datasets.SVHN(
        root="./data", split="train", download=True, transform=train_transform
    )

    # Separate validation dataset using eval transform
...
def build_model(num_classes=10, pretrained=True, freeze_base=True):
    """
    Build and configure ResNet model for SVHN.
    ...
    # Replace final layer for SVHN classification
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        history["patience_counter"] = int(patience_counter)
...
def save_artifacts(
    model, history, class_names, train_results=None, val_results=None, test_results=None
):
    """
    ...
    Args:
        ...
        train_results: Optional train set results
        val_results: Optional validation results
    ...
    # Save test result if available
    if test_results is not None:
        with open(os.path.join(OUTPUT_DIR, "test_results.json"), "w") as f:
            json.dump(convert_to_python_scalars(test_results), f, indent=2)
...
def main():
    ...
    print("=" * 60)
    print("Transfer Learning with ResNet on SVHN")
    print("=" * 60)
    ...
    for key in fine_tune_history:
        if key not in ["best_model_state", "best_val_acc", "patience_counter"]:
            if isinstance(fine_tune_history[key], list):
                history[key].extend([float(x) for x in fine_tune_history[key]])

    if fine_tune_history["best_val_acc"] > history["best_val_acc"]:
        history["best_val_acc"] = float(fine_tune_history["best_val_acc"])
        history["patience_counter"] = int(fine_tune_history["patience_counter"])

    # Evaluate on both train and validations sets
    ...
    sample_loader = DataLoader(
        Subset(test_loader.dataset, list(range(16))), batch_size=16, shuffle=False
    )
    predictions, targets, _ = predict(model, sample_loader)
    ...
    test_acc_pass = test_results["accuracy"] >= thresholds["test_acc_min"]
    val_loss_pass = val_results["loss"] <= thresholds["val_loss_max"]
    beats_random_pass = val_results["accuracy"] >= (
        0.10 + thresholds["beats_random_min"]
    )

    print(
        f"  {'✓ PASS' if val_acc_pass else '✗ FAIL'}: "
        f"Validation Accuracy >= {thresholds['val_acc_min']:.2f} "
        f"(actual: {val_results['accuracy']:.4f})"
    )
    print(
        f"  {'✓ PASS' if test_acc_pass else '✗ FAIL'}: "
        f"Test Accuracy >= {thresholds['test_acc_min']:.2f} "
        f"(actual: {test_results['accuracy']:.4f})"
    )
    print(
        f"  {'✓ PASS' if val_loss_pass else '✗ FAIL'}: "
        f"Validation Loss <= {thresholds['val_loss_max']:.2f} "
        f"(actual: {val_results['loss']:.4f})"
    )
    print(
        f"  {'✓ PASS' if beats_random_pass else '✗ FAIL'}: "
        f"Beats Random Baseline by > 10 points "
        f"(actual: {val_results['accuracy']:.4f})"
    )
    ...
    all_pass = val_acc_pass and test_acc_pass and val_loss_pass and beats_random_pass
